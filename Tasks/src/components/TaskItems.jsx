...
export default (props)=>{
    ...
    useEffect(()=>{
        ...
        if(user){
        axios.post(
            "http://localhost:4000/getTasks",{username:user.uid,list:props.taskId},
          ).then((response) => {
					...
					<div className={classItems.addnewtask}>
						<form>
                            <div className="input-group mb-3">
                                <input type="text" className="form-control  bg-tertiary rounded-3" placeholder="➕ Add new Task" aria-label="tasks" aria-describedby="button-addon2"/>
